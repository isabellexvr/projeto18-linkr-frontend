...
export function LikedButton({ setDisabled, disabled, token, setLiked, liked, e }) {
  return (
    <LikedIcon
      ...
      onClick={() => {
        ...
        dislikeFunction(e.postId, JSON.parse(token), setDisabled);
      }}
    />
  );
}

export function LikeButton({ setDisabled, disabled, token, setLiked, liked, e }) {
  return (
    <LikeIcon
      isRequesting={disabled}
      onClick={() => {
        ...
        postLikeFunction(e.postId, JSON.parse(token), setDisabled);
      }}
    />
  );
}
